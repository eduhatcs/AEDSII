...
public class CaminhoMinimo {
    ...
    public static void heapifyDown(int i) {
	    HeapElemento[] temp = new HeapElemento[heapTamanho+1];
	    tamHeap = 1;

	    while(tampHeap <= heapTamanho) {
		    temp[tamHeap] = heap[tamHeap-1];
		    tamHeap++;
	    }

	    while (i < temp.length/2 && heap[i].distancia > heap[i*2].distancia
	       || heap[i] > heap[i*2+1].distancia) {
		    if (heap[i*2] < heap[i]) {
		    	    swap(i*2], i);
		    } else {
			    swap(i*2+1, i);
